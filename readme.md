...
If a given plan is considered *verified*, the tool will print out: `This plan can be explained by Sabre within the given constraints`. Otherwise, it will print out: `This plan can not be explained by Sabre within the given constraints`. The tool will also print out one more useful piece of information. Most ground actions in Sabre will have character that must consent to them. A *Character-Action Pair* is a tuple of a ground action and one of its consenting characters. The tool will print out the amount of *Character-Action Pairs* in the given plan that the tool was able to generate explanation for.

## Verification

Consider the following plan in the gramma domain as an example, in which the author achieves a utility of 2. We often desire a utility of 2 in this domain as it is the highest utility the author can achieve.
...
This plan can be generated by sabre within reasonable constraints. The tool pushes out this solution as part of its output:
```
walk(Tom, Cottage, Crossroads)
...
```
Character-Action Pairs Explained: 6 / 6
This plan can be explained by Sabre within the given constraints
```

All six *Character-Action Pairs* (one for each action `Tom` takes plus one additional since the `Merchant` also consents to `buy(Tom, Medicine, TomCoin, Market)`) are successfully explained, and the plan is verified.
...
Issue: The author goal was not achieved
Character-Action Pairs Explained: 2 / 2
This plan can be explained by Sabre within the given constraints
```
...
This tool also introduces an aditional optional parameter which allows you to redirect the generated solution into a plain text file. This may be useful as a solution can be parsed by Sabre as a Solution object. Note that only the plan with its explanations will be written into the text file, the other output, including any issues, the *Character-Action Pair* count, and whether the plan could be explained, will still print to standard out.
```
java -jar verify.jar -p gramma.txt -pl actions.txt -o output.txt
```
